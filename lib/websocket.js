...
var wsdecode=function(data) {
	...
	if (length_===0x7E) {
		...
		payload=mask?data.slice(8):data.slice(4);
	} else if (length_===0x7F) {
		length=(data.slice(2,10)).readUInt();
		mask=mask&&data.slice(10,14);
		payload=mask?data.slice(14):data.slice(10);
	} else {
		length=length_;
		mask=mask&&data.slice(2,6);
		payload=mask?data.slice(6):data.slice(2);
	};
...
var wsencode=function(data,type,mask) {
	...
	return [bytes,mask,data].concatBuffers();
};
